...
이 프로젝트의 핵심 목적은 GitOps가 실제로 어떤 흐름으로 동작하는지 직접 구조화해서 이해하는 것입니다. CI가 이미지를 만들고, GitOps repository가 배포 상태의 기준이 되고, ArgoCD가 그 상태를 클러스터에 반영하는 과정을 작은 단위로 구현했습니다. 이렇게 전체 흐름을 직접 연결해두면, 현업에서 더 큰 Kubernetes/GitOps 구조를 만났을 때 각 컴포넌트의 역할과 장애 지점을 더 빠르게 파악할 수 있다고 판단했습니다.

## Why I Built This

GitOps는 단일 도구가 아니라 여러 컴포넌트가 연결된 배포 운영 방식입니다. Jenkins, registry, Helm chart, Git repository, ArgoCD, Kubernetes namespace가 각각 어떤 책임을 갖는지 이해하려면 전체 배포 경로를 직접 구성하는 과정이 필요하다고 판단했습니다.

이 프로젝트에서는 EC2 한 대와 k3s를 선택했습니다. 관리형 Kubernetes를 바로 사용하는 대신 작은 클러스터를 직접 구성하면 Docker, kubeconfig, NodePort, Ingress, namespace, Helm release, ArgoCD Application 같은 기본 요소를 더 선명하게 확인할 수 있기 때문입니다. 규모는 작지만 흐름은 실제 GitOps 배포 구조와 동일하게 가져갔습니다.
...
Jenkins, ArgoCD, registry, 애플리케이션을 한 노드에 함께 올리기 때문에 메모리 여유가 중요합니다. 안정적으로 재현하려면 아래 사양을 기준으로 잡는 것이 좋습니다.
...
EC2 내부 Docker가 registry에 push할 수 있도록 insecure registry를 설정합니다. 이 구성에서는 클러스터 내부 검증을 빠르게 하기 위해 HTTP registry를 사용했습니다.
...
현재 구조를 기준으로 다음 단계에서는 운영 안정성과 보안 수준을 높이는 방향으로 확장할 수 있습니다.
