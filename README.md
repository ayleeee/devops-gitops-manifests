...
CI가 이미지를 만드는 과정을 분리했습니다.
GitOps repository가 배포 상태의 기준이 되도록 구성했습니다.
ArgoCD가 Git의 상태를 클러스터에 반영하도록 연결했습니다.
작은 구조로 먼저 만들어야 큰 Kubernetes 운영 구조도 더 쉽게 읽을 수 있다고 판단했습니다.
...
이 구성에서는 메모리 여유가 중요합니다.
아래 사양을 기준으로 잡으면 안정적으로 재현할 수 있습니다.
...
운영 환경에서는 Jenkins를 그대로 공개하지 않는 편이 좋습니다.
Jenkins는 VPN, bastion, SSO, reverse proxy, HTTPS 뒤에 두는 구성이 더 적합합니다.
...
Jenkins에는 GitHub 접근용 credential을 등록합니다.
credential 값은 README에 기록하지 않습니다.
token 값도 README에 기록하지 않습니다.
...
Jenkins는 메모리를 꽤 사용합니다.
ArgoCD도 메모리를 꽤 사용합니다.
이 구성에서는 4 GiB 이상 인스턴스를 권장합니다.
...
현재 구조는 다음 방향으로 확장할 수 있습니다.
우선순위는 운영 안정성과 보안 수준을 높이는 것입니다.
